...
/* ---------- Benchmark: MSCI World (CHF, währungsgesichert) seit Start ---------- */
const BENCHMARK = { ticker: "IWDC.SW", name: "MSCI World (CHF, gesichert)" };
const BENCHMARK_TTL = 21600; // 6h — Startkurs ist historisch fix, muss nicht oft neu geholt werden
...
// Ersten verfügbaren Schlusskurs ab einem Startdatum holen (nächster Handelstag danach)
async function fetchPriceOnOrAfter(symbol, startDate) {
  const period1 = Math.floor(startDate.getTime() / 1000);
  const period2 = period1 + 10 * 24 * 3600; // 10-Tage-Fenster reicht für den nächsten Handelstag
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(
    symbol
  )}?period1=${period1}&period2=${period2}&interval=1d`;
  const res = await fetch(url, {
    cf: { cacheTtl: BENCHMARK_TTL, cacheEverything: true },
    headers: { "User-Agent": "Mozilla/5.0 (how2invest-dashboard)" },
  });
  if (!res.ok) return null;
  const data = await res.json();
  ...
  for (const c of closes) if (typeof c === "number") return c;
  return null;
}

async function fetchBenchmarkReturn(positions) {
  const dates = positions.map((p) => parseGermanDate(p.kaufdatum)).filter(Boolean);
  if (!dates.length) return null;
  const startDate = new Date(Math.min(...dates.map((d) => d.getTime())));
  const [startPrice, current] = await Promise.all([
    fetchPriceOnOrAfter(BENCHMARK.ticker, startDate).catch(() => null),
    fetchQuote(BENCHMARK.ticker).catch(() => null),
  ]);
  if (startPrice == null || !current) return null;
  return {
    ticker: BENCHMARK.ticker,
    name: BENCHMARK.name,
    startDate: startDate.toISOString().slice(0, 10),
    return: current.price / startPrice - 1,
  };
}
...
async function enrichDepot(name, def) {
  ...
  return {
    ...
    totalReturn,
    benchmark,                                    // {ticker,name,startDate,return} — MSCI World (CHF) seit Start, oder null
